...
showStatus("My[hero:"&$myHero&",hp:"&$myHp&",mostHit:"&$myMostHit&",allHit:"&$myAllHit&",heroHit:"&$myPlayerStatus[$PS_HIT]&"]");
...
EndIf
	...
	If $myHp<13 And ($otherHp-$myMostHit)>18 Then
		If ($otherAllHit-$myAllHit)>5 Then
...
Func generalAIPlayCard($numIteration=2)
	;************
	trace("generalAIPlayCard()...");
	Local $isEnd=False
...
#include<WarlockAIPlayCard.au3>
Func AIPlayCard()
	Return WarlockAIPlayCard()
	Local $hasPlay=False
	Switch($myHero)
		case "Druid"
		case "Hunter"
		...
		case "Warrior"
		case Else
			trace("Unknow hero,use default AIPlay()")
	EndSwitch
	If Not $hasPlay Then
		Return generalAIPlayCard()
	EndIf
	Return false;
EndFunc
