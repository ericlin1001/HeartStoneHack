...
#AutoIt3Wrapper_Outfile=heartStonePlayNew.exe
...
Global $states[]=["mainMenu","battleMode","searchForOppoent","selectCard","playingCard","challenge","arenaMode0","arenaMode1","friendMode","buildingCard","buyingCard","checkForTasks","openCardPack","praticeMode","closed","loseGame","winGame"];
...
Func startApp()
	If _Singleton("heatStonePlay", 1) = 0 Then
	...
	While Not $endWhile
		If isProgramExist("战网") Then
			...
			Else
				...
				clickPics("button_enterGame.bmp|button_enterGame1.bmp")
				Sleep(400)
				Sleep(1300)
			EndIf
		Else
...
Func initScript()
	...
	Next
	For $i=0 to 8
		$heroHashs[$i]=getHash("simgs\hero_"&$heros[$i]&".bmp");
	Next
EndFunc
;***************play game******
Func restMouse()
...
Func mainLoop()
	...
	While 1
		clickPics("button_ok0.bmp|button_ok1.bmp|button_ok2.bmp|button_ok6.bmp")
		$state=getState();
		;*****
...
Func captureAllHandCards()
	...
	updateCurPlayerStatus();
	trace("num hand card:"&$myPlayerStatus[$PS_NUM_HAND_CARDS]);
	For $i=0 to $myPlayerStatus[$PS_NUM_HAND_CARDS]-1
		$x=$myHandCards[$i][$CI_X]
		$y=$myHandCards[$i][$CI_Y]
		mmove($x,$y);
		Local $imgFile=getNextFile('testimgs\card','.bmp');
		Sleep(200)
